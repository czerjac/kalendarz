...
final class Tenis_NET_Kalendarz {
    ...
    public static function shortcode($atts=array()) {
        ...
        <section class="tnk" id="<?php echo esc_attr($id); ?>" data-tnk-calendar>
            <div class="tnk-header"><div><h2>Kalendarz turniejów amatorskich</h2><p class="tnk-lead">Turnieje z TOP PZT, Polskiej Ligi Tenisa, Cuply i Kluby.org w jednym miejscu.</p></div><div class="tnk-meta"><?php if($generated_text):?><span>Aktualizacja danych: <?php echo esc_html($generated_text); ?></span><?php endif;?><?php if(!empty($data['_tnk_stale'])):?><span class="tnk-stale">Pokazujemy ostatnią zapisaną kopię danych.</span><?php endif;?></div></div>
            <div class="tnk-filters" aria-label="Filtry kalendarza">
                <label class="tnk-field tnk-field-search"><span>Szukaj</span><input type="search" data-tnk-filter="search" placeholder="Turniej, miasto, organizator…" autocomplete="off"></label>
                ...
                <label class="tnk-field"><span>Organizacja</span><select data-tnk-filter="organizacja"><option value="">Wszystkie</option><?php foreach($organizacje as $value):?><option value="<?php echo esc_attr($value); ?>"><?php echo esc_html($value); ?></option><?php endforeach;?></select></label>
            ...
            <div class="tnk-table-wrap"><table class="tnk-table"><thead><tr><th>Data</th><th>Turniej</th><th>Miejscowość</th><th>Organizacja</th><th>Kategorie</th><th></th></tr></thead><tbody>
            <?php foreach($items as $index=>$item):$details_id=$id.'-detail-'.$index;$game_types=!empty($item['rodzaje_gry'])&&is_array($item['rodzaje_gry'])?implode('|',$item['rodzaje_gry']):'';$lines=self::details_lines($item);$link=self::details_url($item);?>
                <tr class="tnk-row" data-tnk-item data-start="<?php echo esc_attr(isset($item['data_od'])?$item['data_od']:''); ?>" data-end="<?php echo esc_attr(isset($item['data_do'])?$item['data_do']:''); ?>" data-wojewodztwo="<?php echo esc_attr(isset($item['wojewodztwo'])?$item['wojewodztwo']:''); ?>" data-organizacja="<?php echo esc_attr(isset($item['organizacja'])?$item['organizacja']:''); ?>" data-cykl="<?php echo esc_attr(isset($item['cykl'])?$item['cykl']:''); ?>" data-rodzaje="<?php echo esc_attr($game_types); ?>" data-search="<?php echo esc_attr(self::item_search_text($item)); ?>">
                    <td class="tnk-date"><?php echo esc_html(self::format_date(isset($item['data_od'])?$item['data_od']:'',isset($item['data_do'])?$item['data_do']:'')); ?></td><td><strong class="tnk-title"><?php echo esc_html(isset($item['nazwa'])?$item['nazwa']:''); ?></strong><?php if(!empty($item['cykl'])):?><span class="tnk-cycle"><?php echo esc_html($item['cykl']); ?></span><?php endif;?></td><td><?php echo esc_html(isset($item['miasto'])?$item['miasto']:''); ?></td><td><span class="tnk-source"><?php echo esc_html(isset($item['organizacja'])?$item['organizacja']:(isset($item['zrodlo'])?$item['zrodlo']:'')); ?></span></td><td><?php if(!empty($item['kategorie'])):?><span><?php echo esc_html($item['kategorie']); ?></span><?php endif;?><?php foreach(self::badges($item) as $badge):?><span class="tnk-badge"><?php echo esc_html($badge); ?></span><?php endforeach;?></td><td class="tnk-actions"><button type="button" class="tnk-details-button" aria-expanded="false" aria-controls="<?php echo esc_attr($details_id); ?>" data-tnk-toggle="<?php echo esc_attr($details_id); ?>">Szczegóły</button></td>
                </tr>
                <tr class="tnk-detail-row" id="<?php echo esc_attr($details_id); ?>" data-tnk-detail hidden><td colspan="6"><div class="tnk-detail-box"><dl><?php foreach($lines as $line):?><div><dt><?php echo esc_html($line[0]); ?></dt><dd><?php echo esc_html($line[1]); ?></dd></div><?php endforeach;?></dl><?php if(!empty($item['opis'])):?><p class="tnk-description"><?php echo esc_html($item['opis']); ?></p><?php endif;?><?php if($link):?><a class="tnk-link" href="<?php echo $link; ?>" target="_blank" rel="noopener noreferrer">Szczegóły / zapisy ↗</a><?php endif;?></div></td></tr>
